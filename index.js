...
function getHeroImgPath(name) {
  return 'img/' + name.toLowerCase().replace(/[^a-z]/ig, '') + '.png'
}
...
function buildName() {
  const arr1 = [' адский', ' ржавый', ' черный', 'секретный', ' анальный', ' потный', ' лютый', ' угарный', ' фашистский', ' бесконечный', ' быстрый', ' тактический', ' пивной', ' ZXC', ' терпильный', ' хардкорный']
  const arr2 = [' запой', ' мангус', ' юмор', ' айтем билд', ' позор', ' фид', ' снос трона', ' сон', ' стон', ' радик?', ' пулемет', ' алкоголь', ' трах', ' кач', ' скам', ' тролл фейс', ' рейдж', ' фарм', ' импакт', ' интеллект']
  let str = []
